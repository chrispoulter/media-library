...
export const Sidebar = ({ view, setView }: SidebarProps): React.JSX.Element => {
    return (
        <aside className="flex w-64 flex-col gap-4 bg-zinc-100 p-4 dark:bg-zinc-900 dark:text-white">
            <header>
            ...
            >
                {options.map(({ label, value, icon: Icon }) => (
                    <button
                        ...
                        className={clsx(
                            'flex w-full cursor-pointer items-center gap-3 rounded px-4 py-2 text-sm hover:bg-zinc-200 dark:hover:bg-zinc-800',
